// tests/privacy-detector.test.ts
//
// Parity tests for the ported privacy detector. **Direct port** of the
// canonical TypeScript test matrix at
// `the-librarian/integrations/shared/librarian-lifecycle/tests/privacy.test.ts`.
// Only difference: vitest → bun:test (same describe/it/expect API).
//
// These tests are the parity gate. The same 13 cases live in:
//   - canonical TS: `privacy.test.ts` (the-librarian)
//   - Python port: `tests/test_privacy.py` (the-librarian-hermes-plugin)
//   - JS port: `tests/privacy-detector.test.mjs` (the-librarian-codex-plugin)
//   - this file (the-librarian-opencode-plugin)
//
// A single drift between any of these is a bug — the detector is the
// privacy gate's whole behavioural contract.
